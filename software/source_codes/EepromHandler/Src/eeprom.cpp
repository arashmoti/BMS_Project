...
void EEPROM::ready(void)
{
  ...
  // Wait end of write, but bail out if the bus is hung.
  static uint32_t s_blocked_until_ms = 0;
  static const uint32_t READY_TIMEOUT_MS = 50;       // per-call wait budget
  static const uint32_t READY_RETRY_DELAY_MS = 5000; // pause before retrying after a timeout

  const uint32_t now_ms = (uint32_t)std::chrono::duration_cast<std::chrono::milliseconds>(
                              Kernel::Clock::now().time_since_epoch())
                              .count();

  if (s_blocked_until_ms && now_ms < s_blocked_until_ms)
  {
    _errnum = EEPROM_I2cError;
    return;
  }
  ...
  do {
       ...
       ThisThread::sleep_for(std::chrono::milliseconds(1));
                          ...
                          Kernel::Clock::now().time_since_epoch())
                          .count() - start_ms) < READY_TIMEOUT_MS);

  // Timed out waiting for the device to ACK; flag error and hold off retries briefly.
  _errnum = EEPROM_I2cError;
  s_blocked_until_ms = start_ms + READY_RETRY_DELAY_MS;
